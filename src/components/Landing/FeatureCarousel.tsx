...
export function FeatureCarousel() {
  ...
  return (
    <section
      id="carousel"
      className="carousel relative overflow-hidden border-t border-[#52ADFF]/15 py-20 md:py-28"
      onMouseEnter={() => setPaused(true)}
      onMouseLeave={() => setPaused(false)}
      aria-roledescription="carousel"
      aria-label="Interactive product carousel"
    >
      <div className="pointer-events-none absolute top-1/2 left-1/2 h-[28rem] w-[28rem] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#52ADFF]/12 blur-[120px]" />
...
function CarouselCard({
  ...
  const inner = (
    <>
      <p className="font-mono text-[11px] tracking-[0.22em] text-[#52ADFF] uppercase">
        {slide.step} · {slide.cue}
      </p>
      <h3
        className={cn(
          "font-heading mt-4 font-bold tracking-tight text-[#1A1A1A]",
